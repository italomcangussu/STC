...
ALTER TABLE public.non_socio_students
ADD COLUMN IF NOT EXISTS responsible_socio_id UUID REFERENCES public.profiles(id) ON DELETE SET NULL;

-- 3. Add relationship_type for dependents
ALTER TABLE public.non_socio_students
ADD COLUMN IF NOT EXISTS relationship_type VARCHAR(20) CHECK (relationship_type IN ('filho', 'filha', 'esposo', 'esposa', 'outro'));
...
ALTER TABLE public.non_socio_students
ADD CONSTRAINT non_socio_students_plan_type_check 
CHECK (plan_type IN ('Day Card', 'Card Mensal', 'Dependente'));

-- 5. Create index for faster queries on dependent students
CREATE INDEX IF NOT EXISTS idx_non_socio_students_student_type 
ON public.non_socio_students(student_type);

CREATE INDEX IF NOT EXISTS idx_non_socio_students_responsible_socio 
ON public.non_socio_students(responsible_socio_id);
